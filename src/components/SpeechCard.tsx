    ...
    <div className="rounded-3xl bg-white p-8 shadow-xl ring-1 ring-black/5">
      {loading && (
        <div role="status" aria-live="polite" aria-busy="true" className="animate-pulse space-y-4">
          <span className="sr-only">Indlæser tale for {year}…</span>
      ...
      {!loading && error && (
        <div role="alert" aria-live="assertive" className="text-royal-primary">
      ...
      {!loading && !error && speech && (
        <article className="max-w-none" aria-labelledby={`speech-title-${year}`}>
      ...
      {!loading && !error && !speech && (
        <div role="status" aria-live="polite" className="text-neutral-600">Ingen data at vise.</div>
      )}
    </div>
  )
}
